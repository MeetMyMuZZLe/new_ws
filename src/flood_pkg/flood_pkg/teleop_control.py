...
class TeleopNode(Node):
    ...
    def toggle_fans(self) -> None:
        """
        Toggle the fans on/off
        """
        self.fans_on = not self.fans_on
        joint_state = JointState()
        joint_state.name = ['fan1_joint', 'fan2_joint', 'fan3_joint', 'fan4_joint']
        joint_state.velocity = [10.0 if self.fans_on else 0.0] * 4
        self.joint_state_publisher.publish(joint_state)
        print("Fans toggled", "on" if self.fans_on else "off")
